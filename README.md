...
- `train_temporal_deviation_selector.py`: 训练 temporal AR(1) stale-CSI 下的 learned/DAgger/window/gated temporal deviation selector；学习相对 rotating probe window 的 offset，评估时只使用可观测状态和历史执行反馈。
...
- `evaluate_execution_channel_mismatch.py`: 执行阶段信道失配评估框架，策略先基于 stale/estimated CSI 邀请节点，实际 AirComp slot 再用漂移后的执行信道验证成功；也包含 `stale_topk_feedback` 和 `rotating_feedback_confirm` 这类用当前 aggregate feedback 确认 IRS 选择的 temporal baseline。
...
随后新增 `--feature-mode window`，把每个 offset window 的历史统计显式作为候选特征，并用共享 scorer 给每个 offset 打分：
...
window scorer 有轻微信号，但仍不稳定：offset hit rate 为 `25.49%`；`rho=0.7, delay=1` 下 gap 从 `1.281` 降到 `1.240`、slots 从 `3.690` 降到 `3.597`，`rho=0.98, delay=3` 下 gap 从 `1.004` 降到 `0.966`；但 `rho=0.9, delay=3` 下 window scorer 为 `49.793/50`、perfect `83.00%`、slots `4.070`、gap `1.254`，仍弱于 rotating 的 `49.827/50`、perfect `85.00%`、slots `3.940`、gap `1.242`。因此结论是：显式 window 表示比纯全局特征更合理，但只靠历史统计 reranking 还不足以稳定吃到 oracle gap。

随后新增 confidence-gated window deviation：当模型预测的最佳 offset 相对 rotating offset 的分数优势小于阈值时，直接回退到 offset `0`。同时固定 PyTorch seed，保证训练初始化和 DataLoader shuffle 可复现：
...
gated pilot 的 validation offset hit rate 为 `28.01%`，但闭环仍没有稳定超过 rotating。高阈值 `m=0.05` 多数场景基本退回 rotating；低阈值仍会牺牲 perfect rate 或 slots。例如 `rho=0.9, delay=3` 下，rotating 为 `49.827/50`、perfect `85.00%`、slots `3.940`、gap `1.242`；raw window 为 `49.787/50`、perfect `81.33%`、slots `4.067`、gap `1.240`；`m=0.05` gated 回到 `49.827/50`、perfect `85.00%`、slots `3.940`、gap `1.242`。结论是：margin gate 可以作为回退护栏，但当前模型分数没有足够校准，无法把 oracle gap 稳定转化成可部署收益。

随后转向显式 feedback confirmation，并在 `evaluate_execution_channel_mismatch.py` 中新增 `stale_topk_feedback`。先做过一个无 feedback 的 stale top-K 诊断：只要完整扫描 stale codebook 并把 stale top-1 放入候选集，最终选择就会退化为 `Exact Greedy Full CSI`，因此不能作为真正的有限 CSI 结果。当前保留的 confirmed 版本做法是：用完整 stale codebook 排序取半个预算、再用 rotating coverage 填满 B 个 IRS 候选；随后只在这 B 个候选上读取当前 aggregate feedback（可调度比例和平均功率），用 feedback 选 IRS index，但节点邀请 mask 仍来自 stale/decision CSI。
...
这个 pilot 给出正向但带成本的信号：跨 9 个 rho/delay 场景，`Rotating B=4` 平均 `49.999/50`、perfect `99.89%`、slots `3.937`、gap `1.145`、preview `4`；`Stale-TopK Feedback B=4` 为 `49.996/50`、perfect `99.63%`、slots `3.458`、gap `0.497`、preview `20`；`Temporal Deviation Oracle B=4` 为 `50.000/50`、slots `3.011`、gap `0.359`、preview `4`。例如 `rho=0.9, delay=3` 下，rotating slots/gap 为 `4.210/1.187`，confirmed feedback 为 `3.787/0.556`，deviation oracle 为 `3.170/0.403`。结论是：当前 aggregate feedback confirmation 可以明显缩小 rotating gap，但它不是等预算 B=4 策略；下一步应研究如何把完整 stale 扫描成本降下来，或把 B 个当前 feedback probes 变成真正的主动 probe-set selection / confidence allocation。

随后新增低成本对照 `rotating_feedback_confirm`：仍只取普通 rotating 的 B 个 IRS 候选，节点邀请 mask 来自 stale/decision CSI，然后对这 B 个候选读取当前 aggregate feedback 来选择 IRS index。该策略在 `B=4` 时成本为 `Preview=8`，即 `4` 个 stale preview 加 `4` 个当前 feedback probes。pilot 结果是负面的：跨 9 个 rho/delay 场景，`Rotating Feedback Confirm B=4` 平均 slots `4.446`、gap `1.210`，均弱于普通 rotating 的 slots `3.937`、gap `1.145`；虽然 failed invitations 从 `1.66` 降到 `0.67`，missed opportunities 却从 `1.17` 升到 `2.88`。这说明只在固定 rotating 集合内部做 current feedback confirmation 不够，`Stale-TopK Feedback` 的收益主要来自更好的候选集生成，而不是 confirmation 本身。
...
- Window scorer 用每个 offset window 的历史统计做共享打分，能小幅降低部分场景 gap，但仍不稳定：`rho=0.9, delay=3` 下 window scorer 为 `49.793/50`、perfect `83.00%`、gap `1.254`，仍弱于 rotating。下一步应加入 uncertainty-aware guardrail 或 feedback confirmation，避免为了小幅 gap 改善牺牲 perfect rate 和 slots。
- Confidence-gated window deviation 已验证：PyTorch 训练 seed 已固定，`--gate-margin-thresholds 0,0.005,0.01,0.02,0.05` 下，高阈值基本退回 rotating，低阈值仍不稳定。下一步应停止纯历史统计 reranking，转向显式 feedback confirmation / active probe confirmation。
- Stale-TopK Feedback 是第一个正向可部署确认基线，但预算不等价：它使用完整 stale codebook 排序加 B 个当前 aggregate feedback probes。9 个 rho/delay 场景平均看，slots 从 `Rotating B=4` 的 `3.937` 降到 `3.458`，gap 从 `1.145` 降到 `0.497`，但 preview 从 `4` 升到 `20`。
- Rotating Feedback Confirm 是低成本确认的负结果：它只在普通 rotating B 个候选内部做当前 aggregate feedback confirmation，`Preview=8`，failed 从 `1.66` 降到 `0.67`，但 missed 从 `1.17` 升到 `2.88`，slots/gap 变成 `4.446/1.210`，弱于普通 rotating 的 `3.937/1.145`。这说明下一步不能只加 confirmation，而要优先改 candidate generation。
