...
- `evaluate_execution_channel_mismatch.py`: 执行阶段信道失配评估框架，策略先基于 stale/estimated CSI 邀请节点，实际 AirComp slot 再用漂移后的执行信道验证成功；也包含 `stale_topk_feedback` 这种用当前 aggregate feedback 确认 IRS 选择的 temporal baseline。
...
- Stale-TopK Feedback 是第一个正向可部署确认基线，但预算不等价：它使用完整 stale codebook 排序加 B 个当前 aggregate feedback probes。9 个 rho/delay 场景平均看，slots 从 `Rotating B=4` 的 `3.937` 降到 `3.458`，gap 从 `1.145` 降到 `0.497`，但 preview 从 `4` 升到 `20`。下一步应把它改造成真正低成本的 active probe confirmation。
